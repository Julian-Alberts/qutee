...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    for i in [100, 1_000, 10_000, 100_000] {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(format!("{} Elements", i)), &data[0..i as usize], |b, d| {
            b.iter(|| {
                let mut tree: QuadTree<_,_,ConstCap<16>> = QuadTree::new_with_const_cap(Boundary::between_points((0, 0), (32_767, 32_767)));
    ...
    for i in [((0,0), (32_767, 32_767)), ((500,500), (25_000, 25_000)), ((15_000, 15_000), (20_000, 20_000))] {
        let area = (i.1.0 - i.0.0) * (i.1.1 - i.0.1);
        let query = Boundary::between_points(i.0, i.1);
        group.bench_with_input(BenchmarkId::from_parameter(format!("{query} Area: {area}")), &query, |b, q| {
            b.iter(|| tree.query(q.clone()).count())
        });
    }
    group.finish();
}
